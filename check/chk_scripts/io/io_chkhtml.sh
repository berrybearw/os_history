...
$(document).ready(function() {
   ...
   var title = {' > io.html
echo "
      text: '$where_ip iowait 歷史使用率 $test_date '   
   };
   var subtitle = {
      text: 'iowait 100% '
   };
...
cat $test_dir/${file}_date.txt | sed '$d' >> io.html
...
cat $test_dir/${file}_iowait.txt | sed '$d' >> io.html
...
cat $test_dir/${file}_iotest.html >> io.html
...
if [ "$MODE" == "check" ] ; then
   cp -p ${chk_dir}/chk/$name/io.html $TEMPDIR/io_${name}.html

   cd ${chk_dir}/chk
   rm -rf $name

   echo "請下載檔案 " $TEMPDIR/io_${name}.html
else
   cp -p $chk_dir/io.html $TEMPDIR/io.html
   echo "請下載檔案 " $TEMPDIR/io.html
fi
