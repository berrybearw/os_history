...
if [ ! -d "$chk_dir/month/$file" ]; then
   mkdir $chk_dir/month/$file
fi
...
#fi
#
#proc_not_to='db_space_chk.sh|db_space_chkhtml.sh'
#
#if [ "$tmon_clean_time_go" == "Y" ]; then
#
#   #找出指定時間目錄 清理
#   find $chk_dir/* -mtime +$chk_clean -print0 | grep -Ev "(grep|${proc_not_to})" #| xargs -0 -l1 -t rm -rf
#
#fi
